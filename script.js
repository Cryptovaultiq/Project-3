document.addEventListener('DOMContentLoaded', () => {
    ...
    // Check for critical elements and log specific missing ones
    const missingElements = [];
    if (!walletSelect) missingElements.push('walletSelect');
    if (!connectButton) missingElements.push('connectButton');
    if (!connectManuallyButton) missingElements.push('connectManuallyButton');
    if (!manualConnectSection) missingElements.push('manualConnectSection');
    if (!userInput) missingElements.push('userInput');
    if (!submitButton) missingElements.push('submitButton');
    if (!coinmarketcapData) missingElements.push('coinmarketcapData');
    if (!hamburger) missingElements.push('hamburger');
    ...
    document.querySelectorAll('.nav-menu a').forEach(anchor => {
        anchor.addEventListener('click', (e) => {
            ...
            if (targetId === 'home' || targetId === 'connect-wallet') {
                showPage(targetId); // Navigate to home or connect-wallet
            } else {
    ...
    connectButtons.forEach(button => {
        button.addEventListener('click', () => {
            console.log('Card button clicked:', button.getAttribute('data-option')); // Debug log
            showPage('connect-wallet'); // All cards navigate to Page 2
        });
    });
    ...
    connectButton.addEventListener('click', () => {
        ...
        if (walletSelect.value) {
            showPage('initializing'); // Connect button navigates to Page 3
            setTimeout(() => {
                showPage('error-connecting'); // Navigate to Page 4 after 3 seconds
            }, 3000);
        }
    });
